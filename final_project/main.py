...
def test(siamese_gnn, logger, gen): ## LUCA
    labels = (Variable(torch.arange(0, gen.N).unsqueeze(0).expand(gen.num_examples_test,
              gen.N)).type(dtype_l))
    input = gen.sample_batch(gen.num_examples_test, cuda=torch.cuda.is_available(), is_training=False)
    pred = siamese_gnn(*input)
    loss = compute_loss(pred, labels)
    logger.add_test_accuracy(pred, labels)
    print(logger.accuracy_test[-1],)
...
if __name__ == '__main__':
    ...
    elif args.mode == 'test': ## Luca
        test(siamese_gnn, logger, gen) ## Luca
